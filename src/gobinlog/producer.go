...
func NewProducer(c *Config, mysqlcfg MysqlConfig) (*Producer, error) {
	...
	if canalIns, err := canal.NewCanal(cfg); err != nil {
        ...
        kafkaconfig.Producer.Retry.Max = 10                   // Retry up to 10 times to produce the message
        ...
        log.Info("kafka addrs is ",addrs)
        p.rdkafka,err = sarama.NewSyncProducer(addrs, kafkaconfig)
		if err != nil {
			log.Errorf("Get kafkaclient failed: %v", err)
			return nil, err
        }

        p.topic = c.ClusterConfig.Topic
		p.sqlcfg = mysqlcfg
		//注册RowsEventHandler
		...
		log.Infof("Producer start ok. id = %s", p.sqlcfg.Id)
		return p, nil
	}
}
...
func (p *Producer) Close() {
	...
	p.waitGroup.Wait()
    p.rdkafka.Close()
    log.Info("Producer safe close and exit id = %s", p.sqlcfg.Id)
}
...
func (p *Producer) updateSql(msg Message) int {
	...
	if msg.Action == "update" {
        ...
        var sqlkey interface{}
        if len(msg.PrimaryKeys[0]) != 0 {
            sqlkey = msg.PrimaryKeys[0][0]
        }
        primary_keys := sqlkey //初始值主键是内容需要根据内容查找主键
        ...
        //若对无主键的表 update 则拼接上所有字段
        if sqlMid == " in " {
            sqlStart = sqlStart + " where"
            for _, array := range msg.RawRows {
                for k,v := range array {
                    sqlStart = fmt.Sprint(sqlStart," ",k," = ")
                    if typeof(v) == "string" {
                        sqlStart = fmt.Sprint(sqlStart,"'",v,"'")
                    } else if typeof(v) == "num" {
                        sqlStart = fmt.Sprint(sqlStart,v)
                    } else {
                        log.Errorf("can not know get value type",typeof(v))
                    }
                    sqlStart = fmt.Sprint(sqlStart, " and")
                }
            sqlStart=sqlStart + ";"
            sqlStart = strings.Replace(sqlStart, " and;", ";", -1)
            sql = sqlStart
            }
        } else {
...
func (p *Producer) deleteSql(msg Message) int {
	...
	if msg.Action == "delete" {
        sqlStart := fmt.Sprintf("delete from %s.%s ", msg.Schema, msg.Table)
        var sqlkey interface{}
        if len(msg.PrimaryKeys[0]) != 0 {
            sqlkey = msg.PrimaryKeys[0][0]
        }
		...
		pk_type_after := typeof(primary_keys)
		if pk_type_after == "num" {
			sqlStart = sqlStart + fmt.Sprintf("where %d ", primary_keys)
		} else if pk_type_after == "string" {
			sqlStart = sqlStart + fmt.Sprintf("where %s ", primary_keys)
		}
...
func (p *Producer) sqlProcessing(id int64, quary string, schema string) error {
    ...
    _, _, err := p.rdkafka.SendMessage(&sarama.ProducerMessage{Topic: string(p.topic),Value: sarama.StringEncoder(quary),})
     if err != nil {
		log.Errorf("qpush failed: %v kafka topic is [%v]", err,p.topic)
		p.Close()
		return err
    }
